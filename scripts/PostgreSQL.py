def PostgreSQL():
    ...
    encode_user = user.encode("UTF-8").hex()
    encode_db = db.encode("UTF-8").hex()
    encode_query = query.encode("UTF-8").hex()
    len_query = len(query) + 5

    start = "000000" + str(chr(4+len(user)+8+len(db)+13).encode("UTF-8").hex()) + "000300"
    data = "00" + "user".encode("UTF-8").hex() + "00" + encode_user + "00" + "database".encode("UTF-8").hex() + "00" + encode_db
